...
final class AutoStartSessionWindowTests: XCTestCase {
    ...
    func testEarlyMorningWindowAllowsLaterMorningAutoStart() {
        let window = AutoStartSessionWindow(startMinuteOfDay: (5 * 60) + 30, endMinuteOfDay: (23 * 60) + 30)

        XCTAssertFalse(window.contains(date(hour: 5, minute: 29), calendar: calendar))
        XCTAssertTrue(window.contains(date(hour: 5, minute: 30), calendar: calendar))
        XCTAssertTrue(window.contains(date(hour: 7, minute: 30), calendar: calendar))
        XCTAssertFalse(window.contains(date(hour: 23, minute: 30), calendar: calendar))
    }
    ...
    func testNextStartForEarlyMorningWindowSchedulesSameMorning() {
        let window = AutoStartSessionWindow(startMinuteOfDay: (5 * 60) + 30, endMinuteOfDay: (23 * 60) + 30)

        XCTAssertEqual(
            window.nextStart(after: date(hour: 1, minute: 30), calendar: calendar),
            date(hour: 5, minute: 30)
        )
    }

    func testNextStartReturnsCurrentDateWhenAlreadyInsideWindow() {
        let window = AutoStartSessionWindow(startMinuteOfDay: (5 * 60) + 30, endMinuteOfDay: (23 * 60) + 30)
        let currentDate = date(hour: 7, minute: 30)

        XCTAssertEqual(window.nextStart(after: currentDate, calendar: calendar), currentDate)
    }

    func testNextStartForDaytimeWindowSchedulesTomorrowAfterWindowEnds() {
        let window = AutoStartSessionWindow(startMinuteOfDay: 6 * 60, endMinuteOfDay: 18 * 60)

        XCTAssertEqual(
            window.nextStart(after: date(hour: 19), calendar: calendar),
            date(day: 13, hour: 6)
        )
    }

    func testNextStartForOvernightWindowSchedulesSameDayBetweenEndAndStart() {
        let window = AutoStartSessionWindow(startMinuteOfDay: 22 * 60, endMinuteOfDay: 2 * 60)

        XCTAssertEqual(
            window.nextStart(after: date(hour: 3), calendar: calendar),
            date(hour: 22)
        )
    }
