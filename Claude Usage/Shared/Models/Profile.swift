...
struct AutoStartSessionWindow: Codable, Equatable {
    ...
    // Backwards-compatible decoding for windows saved before `isEnabled` existed
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        isEnabled = try container.decodeIfPresent(Bool.self, forKey: .isEnabled) ?? true
        startMinuteOfDay = Self.clampedMinuteOfDay(try container.decode(Int.self, forKey: .startMinuteOfDay))
        endMinuteOfDay = Self.clampedMinuteOfDay(try container.decode(Int.self, forKey: .endMinuteOfDay))
    }
    ...
    func nextStart(after date: Date, calendar: Calendar = .current) -> Date {
        if contains(date, calendar: calendar) {
            return date
        }

        let startOfToday = calendar.startOfDay(for: date)
        let todayStart = calendar.date(byAdding: .minute, value: startMinuteOfDay, to: startOfToday) ?? date

        if date < todayStart {
            return todayStart
        }

        return calendar.date(byAdding: .day, value: 1, to: todayStart) ?? date.addingTimeInterval(24 * 60 * 60)
    }
...
struct Profile: Codable, Identifiable, Equatable {
    ...
    func allowsAutoStartSession(at date: Date = Date(), calendar: Calendar = .current) -> Bool {
        guard let autoStartSessionWindow, autoStartSessionWindow.isEnabled else {
            return true
        }

        return autoStartSessionWindow.contains(date, calendar: calendar)
    }
}
